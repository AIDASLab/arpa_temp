...
# Train on ADNI_VQA_CVRF_GDS_AGE (age regression) and evaluate on its test split.
TRAIN_KEYS=${1:-ADNI_VQA_CVRF_GDS_AGE}
EVAL_KEYS=${2:-$TRAIN_KEYS}
FINETUNE_MODE=${3:-${FINETUNE_MODE:-lora}}
GENERATION_EVAL=${GENERATION_EVAL:-true}
GENERATION_EVAL_INTERVAL=${GENERATION_EVAL_INTERVAL:-100}
...
VISION_FULL_FT=${VISION_FULL_FT:-true}
...
REGRESSION_HUBER_BETA=${REGRESSION_HUBER_BETA:-1.0}
NORMALIZED=${NORMALIZED:-false}

export CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-0,1,2,3,4,5,6,7}
export WANDB_API_KEY=${WANDB_API_KEY:-9c2e7fba1823550f9d2f28b6d6e141982aeb7e3b}
...
default_output_dir_for_key() {
  case "$1" in
    ADNI_VQA_CVRF_GDS_AGE)
      echo "output/ys_adni_vqa_cvrf_gds_age_lora_128_log"
      ;;
    *)
      local safe
...
for idx in "${!TRAIN_KEYS_ARRAY[@]}"; do
  ...
  loss_suffix=$(build_regression_head_suffix)
  wandb_run_name="cvrf_gds_age_${safe_key}_${loss_suffix}"
  ...
  if [[ "${VISUAL_PROJ_FULL_FT}" == "true" ]]; then
    TRAIN_CMD+=(--visual_projector_full_finetune --visual_projector_modules "${VISUAL_PROJ_MODULES}")
  fi
  if [[ "${NORMALIZED}" == true ]]; then
  ...
  if [[ "${GENERATION_EVAL}" == "true" ]]; then
    TRAIN_CMD+=(--generation_eval_per_epoch --generation_eval_interval_epochs "${GENERATION_EVAL_INTERVAL}")
  else
