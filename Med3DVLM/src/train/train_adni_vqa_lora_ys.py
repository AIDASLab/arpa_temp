...
def profile_flops_for_step(
    model: nn.Module,
    batch: Dict[str, torch.Tensor],
    device: torch.device,
    autocast_dtype: Optional[torch.dtype] = None,
) -> float:
    """Measure forward+backward FLOPs for a single batch."""
    model.train()
    ...
    total_flops = sum(evt.flops for evt in prof.function_events if getattr(evt, "flops", None))
    return float(total_flops or 0.0)


def profile_flops_over_batches(
    model: nn.Module,
    dataset: Dataset,
    data_collator,
    batch_size: int,
    ...
    """Run a few warm-up batches to estimate FLOPs per training step. Returns average FLOPs."""
    if steps <= 0:
        return None
    # Move model to the target device before profiling to avoid device mismatch.
    try:
    ...
    for idx in range(steps):
        try:
            ...
            break
        flops = profile_flops_for_step(model, batch, device, autocast_dtype)
        total_flops += flops
        ran += 1
        if not is_dist_initialized() or get_rank() == 0:
            print(f"[flops] batch {idx + 1}: {flops / 1e12:.4f} TFLOPs forward+backward")
    if ran > 0 and (not is_dist_initialized() or get_rank() == 0):
        avg = total_flops / ran
        print(f"[flops] average over {ran} batch(es): {avg / 1e12:.4f} TFLOPs per step")
        return avg
    model.zero_grad(set_to_none=True)
    if device.type == "cuda":
...
class WandbLossConfigCallback(TrainerCallback):
    """
    Log regression loss settings to wandb config so changed loss choices are visible in the run.
    """
    ...
    def _should_log(self, state) -> bool:
        ...
        if wandb.run is None:
            return False
        is_zero = getattr(state, "is_world_process_zero", True)
        if is_dist_initialized() and not is_zero and get_rank() != 0:
            return False
        return True
    ...
    def on_train_begin(self, args, state, control, **kwargs):
        ...
        try:
            ...
            if not is_dist_initialized() or get_rank() == 0:
                print(f"[wandb] Failed to log loss config: {err}")


class WandbTrainLossLogger(TrainerCallback):
    """
    Ensure per-step training loss/learning rate are pushed to wandb even if Trainer skips logging them.
    """
...
def train(args: argparse.Namespace) -> None:
    ...
    profiled_flops: Optional[float] = None
    profiled_flops_tflops: Optional[float] = None
    if args.profile_flops:
        autocast_dtype = torch.float16 if use_fp16 else (torch.bfloat16 if use_bf16 else None)
        profiled_flops = profile_flops_over_batches(
            model=model,
            dataset=train_dataset,
            data_collator=data_collator,
            batch_size=args.batch_size,
            device=training_args.device,
            steps=max(1, int(args.profile_flops_steps)),
            autocast_dtype=autocast_dtype,
        )
        if profiled_flops is not None:
            profiled_flops_tflops = profiled_flops / 1e12
    ...
    if args.use_wandb:
        callbacks.append(WandbTrainLossLogger())
        if is_regression_task:
            loss_config = {
                "regression_loss_type": args.regression_loss_type,
                "regression_mle_variance": args.regression_mle_variance,
                "regression_huber_beta": args.regression_huber_beta,
                "regression_normalized": bool(args.normalized),
                "train_dataset_keys": train_keys,
                "eval_dataset_keys": eval_keys,
            }
            callbacks.append(WandbLossConfigCallback(loss_config))
    ...
    if profiled_flops is not None and (not is_dist_initialized() or get_rank() == 0):
        # Surface FLOPs estimate into Trainer logging (console + WandB) at step 0.
        flops_logs = {
            "flops/per_step": profiled_flops,
            "flops/per_step_tflops": profiled_flops_tflops,
        }
        print(
            f"[flops] estimated per-step: {profiled_flops_tflops:.4f} TFLOPs (raw: {profiled_flops:.0f})",
            flush=True,
        )
        trainer.log(flops_logs)
        if wandb.run is not None:
            wandb.log(flops_logs, step=0)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--profile_flops",
        action="store_true",
        help="Run a short forward+backward profiling pass before training to estimate FLOPs per step.",
    )
    parser.add_argument(
        "--profile_flops_steps",
        type=int,
        default=1,
        help="Number of mini-batches to average when computing FLOPs (requires --profile_flops).",
    )
    parser.add_argument(
